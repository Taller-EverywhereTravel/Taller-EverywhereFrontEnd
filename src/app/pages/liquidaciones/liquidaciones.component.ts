  ...
  private async loadDetallesLiquidacion(liquidacionId: number): Promise<void> {
    try {
      ...
      this.detalles = detalles.map(detalle => ({
        ...
        viajero: detalle.traveler,
        ticket: detalle.ticket,
        documentoCobro: detalle.documentCollection,
        costoTicket: detalle.costTicket,
        cargoServicio: detalle.chargeService,
        valorVenta: detalle.valueSale,
        feeEmision: detalle.feeEmision,
        documentoFee: detalle.documentFee,
        comision: detalle.comission,
        facturaCompra: detalle.invoicePurchase,
        boletaPasajero: detalle.ticketPassenger,
        montoDescuento: detalle.amountDiscount,
        pagoPaxUSD: detalle.paymentPaxUSD,
        pagoPaxPEN: detalle.paymentPaxPEN,
        isTemporary: false
      }));
    } catch (error) {
